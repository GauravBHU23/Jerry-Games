...
async function handlePossibleWin(playerId, run) {
  try {
    ...
    if (!acc || !acc.email) return { sent: false, reason: 'no account email' };

    // first finish only - claimWin is atomic, so a double submit sends once
    if (!(await backend.claimWin(playerId))) {
      return { sent: false, reason: 'already won' };
    }

    const board = await buildLeaderboard();
    const entry = board.find(r => r.id === playerId);

    const { sendWinnerMail } = require('./mailer');
    return await sendWinnerMail({
      to: acc.email,
