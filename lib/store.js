...
async function pg() {
  ...
  // Set the first time a player clears level 8, so the winner email goes out
  // once however many times they finish it again.
  await sql`ALTER TABLE accounts ADD COLUMN IF NOT EXISTS won_at TIMESTAMPTZ`;

  // Wins by players with no account (they signed up before an email was
  // required). Kept separate so the alert still fires exactly once for them.
  await sql`CREATE TABLE IF NOT EXISTS legacy_wins (
...
const pgStore = {
  ...
  async findAccountById(id) {
    const db = await pg();
    const [row] = await db`SELECT * FROM accounts WHERE id = ${id} LIMIT 1`;
    return row || null;
  },
  ...
  // Claims the win for this player. Returns true only for the caller that got
  // there first, so the email cannot be sent twice.
  async claimWin(id) {
    const db = await pg();
    const rows = await db`UPDATE accounts SET won_at = now()
                          WHERE id = ${id} AND won_at IS NULL
                          RETURNING id`;
    return rows.length > 0;
  },

  async claimLegacyWin(id) {
    const db = await pg();
    // ON CONFLICT DO NOTHING makes this a one-shot claim even under a race
    const rows = await db`INSERT INTO legacy_wins (player_id) VALUES (${id})
                          ON CONFLICT (player_id) DO NOTHING
                          RETURNING player_id`;
    return rows.length > 0;
  },

  // Removes a player everywhere: their runs, their account and their board row.
  async deletePlayer(id) {
    const db = await pg();
    const scores = await db`DELETE FROM scores WHERE player_id = ${id} RETURNING id`;
    const accounts = await db`DELETE FROM accounts WHERE id = ${id} RETURNING id`;
    const players = await db`DELETE FROM players WHERE id = ${id} RETURNING id`;
    await db`DELETE FROM legacy_wins WHERE player_id = ${id}`;
    return { scores: scores.length, accounts: accounts.length, players: players.length };
  },
};
...
// `wonAt` is set the first time this player clears level 8.
const ACCOUNTS_HEADER = 'id,email,name,username,pass,created,wonAt';
// wins by players who have no account row to mark
const LEGACY_WINS_CSV = path.join(MEDIA, 'legacy-wins.csv');
const LEGACY_WINS_HEADER = 'id,wonAt';
...
const csvStore = {
  ...
  async findAccountById(id) {
    return readCsv(ACCOUNTS_CSV, ACCOUNTS_HEADER).find(a => a.id === id) || null;
  },
  ...
  async claimWin(id) {
    const rows = readCsv(ACCOUNTS_CSV, ACCOUNTS_HEADER);
    const a = rows.find(r => r.id === id);
    if (!a || a.wonAt) return false;     // already claimed - do not mail again
    a.wonAt = new Date().toISOString();
    writeAccounts(rows);
    return true;
  },

  async claimLegacyWin(id) {
    const rows = readCsv(LEGACY_WINS_CSV, LEGACY_WINS_HEADER);
    if (rows.some(r => r.id === id)) return false;
    appendCsv(LEGACY_WINS_CSV, LEGACY_WINS_HEADER, [id, new Date().toISOString()]);
    return true;
  },

  async deletePlayer(id) {
    const scores = readCsv(SCORES_CSV, SCORES_HEADER);
    const keptScores = scores.filter(s => s.id !== id);
    fs.writeFileSync(SCORES_CSV, SCORES_HEADER + '\n' +
      keptScores.map(r => [r.id, r.name, r.score, r.level, r.durationMs, r.mode, r.playedAt]
        .map(csvEscape).join(',')).join('\n') + (keptScores.length ? '\n' : ''), 'utf8');

    const accounts = readCsv(ACCOUNTS_CSV, ACCOUNTS_HEADER);
    const keptAccounts = accounts.filter(a => a.id !== id);
    writeAccounts(keptAccounts);

    const players = readCsv(PLAYERS_CSV, PLAYERS_HEADER);
    const keptPlayers = players.filter(p => p.id !== id);
    fs.writeFileSync(PLAYERS_CSV, PLAYERS_HEADER + '\n' +
      keptPlayers.map(r => [r.id, r.name, r.created, r.lastSeen]
        .map(csvEscape).join(',')).join('\n') + (keptPlayers.length ? '\n' : ''), 'utf8');

    return {
      scores: scores.length - keptScores.length,
      accounts: accounts.length - keptAccounts.length,
      players: players.length - keptPlayers.length,
    };
  },
};
...
function writeAccounts(rows) {
  const body = rows.map(r =>
    [r.id, r.email, r.name, r.username, r.pass, r.created, r.wonAt || '']
      .map(csvEscape).join(',')).join('\n');
...
const missingDb = {
  ...
  async claimWin() { return false; },
  async claimLegacyWin() { return false; },
  async deletePlayer() { throw new Error('Database not configured.'); },
...
// Called when a run comes in at level 8. Claims the win (once per account)
// and emails the player. Anything that goes wrong here is logged and
// swallowed - a mail problem must never fail a player's score submission.
const FINAL_LEVEL = 8;
async function handlePossibleWin(playerId, run) {
  try {
    if (!run || Number(run.level) < FINAL_LEVEL) return { sent: false, reason: 'not a win' };

    // The account is optional here. Older players signed up with a name only,
    // and the organisers still want to hear about every finish, so a missing
    // account must not stop the alert.
    const acc = await backend.findAccountById(playerId);
    const player = acc || await backend.getPlayer(playerId);
    if (!player) return { sent: false, reason: 'player not found' };

    // First finish only. claimWin is atomic on an account; players without one
    // fall back to a marker row so they cannot alert twice either.
    const claimed = acc ? await backend.claimWin(playerId)
                        : await claimWinWithoutAccount(playerId);
    if (!claimed) return { sent: false, reason: 'already won' };

    const board = await buildLeaderboard();
    const entry = board.find(r => r.id === playerId);
    // The name the leaderboard shows is the authority, so the mail always
    // greets people by the name they see next to their rank. Falls back to
    // the account, then the player row, for a first win that is not on the
    // board yet.
    const username = (entry && entry.name) ||
                     (acc && acc.username) ||
                     player.name || 'unknown';
    const playerEmail = acc && acc.email ? acc.email : null;
    ...
    const { sendWinnerMail, sendOrganiserAlert } = require('./mailer');

    // The organisers hear about every win, whether or not the player can be
    // reached; the player only gets one if they have an address.
    const alert = await sendOrganiserAlert(details);
    const winner = playerEmail
      ? await sendWinnerMail({ ...details, to: playerEmail })
      : { sent: false, reason: 'player has no email' };

    return { sent: winner.sent, winner, alert, username, playerEmail };
  } catch (e) {
    console.error('[mail] winner handling failed:', e.message);
    return { sent: false, reason: e.message };
  }
}

// Players from before signup required an email have no accounts row to mark.
// Their win is recorded in a table of its own, because an in-memory flag would
// be forgotten every time a serverless function cold-starts and the alert
// would fire again on the next submission.
async function claimWinWithoutAccount(playerId) {
  return backend.claimLegacyWin(playerId);
}
